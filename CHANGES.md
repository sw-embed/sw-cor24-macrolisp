...
- fix `demos/lazy.l24` hang + repurpose `fullbig`. Root cause of the lazy hang: `lazy-take` built `(cons (lazy-car s) (lazy-take …))` in **non-tail** position, growing the eval C-stack ~1 KB/element and overflowing the 3 KB EBR default at n=3 (silent truncation under `cor24-emu`; infinite loop under old `cor24-run`). Pre-existing — reproduces before the tail-recursive prelude rewrites (bisected to parent of `ef50c70`), so not migration fallout. Fix: rewrote `lazy-take` with an accumulator + `reverse` (`lazy-take-helper`, matching the `take-helper` pattern) in `src/prelude-full.h`, `src/main.c`, and `prelude/full.l24`; now bounded and correct at the 3 KB default (verified to n=20+). `fullbig`: the SRAM-relocation `_start` patch (`sp=0x100000`) tripped `cor24-emu`'s EBR-only stack guard and halted after 2 instructions; SRAM stacks aren't supported by the emulator and no demo needs >8 KB, so `eval-fullbig` now runs the full prelude at `--stack-kilobytes 8` (the EBR ceiling) and `build-fullbig` is removed. Added regression checks to `scripts/run-tests.sh` (`lazy ok`: lazy.l24 completes at the 3 KB default; `fullbig ok`: eval-fullbig runs past init). Docs updated (`required-stack-changes.md`). No emulator changes.
